...
public class BankFunctions {
    ...
    public int createAcc() {
        ...
        do {
            ...
            if (duplicated == true)
                System.out.println("ID account is existed!!!");
        } while (duplicated == true);

        do {
            account_name = tool.acceptName("Enter your name: ");
            if (account_name == null)
                System.out.println("Name does not contain special characters or admin!!");
        } while (account_name == null);

        do {
            do {
                password = tool.acceptPassword(
                        "Enter password  at least 6 characters, including uppercase letters, lower case letters, numeric characters and 1 special character:  ");
                if (password == null)
    ...
    public int depositMoney(BankAccount curAccount) {
        ...
        do {
            try {
                ...
                if (confirm.equalsIgnoreCase("yes") || confirm.equalsIgnoreCase("Yes") || confirm.equalsIgnoreCase("y")
                        || confirm.equalsIgnoreCase("Y"))
                    return list.depositMoney(curAccount, depositMoney);
                else
                    return result = -3;
            } catch (Exception e) {
                // TODO: handle exception
    ...
    public int transferMoney(BankAccount curAccount) {
        if (curAccount.getBalance() >= 1) {
            ...
            if (list.findAccount(receiveAccountId) == null)
                return -2;
            else {
                transferMoney = tool.acceptMoney("Enter money you want to transfer: ");
                if (transferMoney > curAccount.getBalance())
                    return -1;
                else {
                    System.out.print("Do you want to transfer ?? (Yes/No): ");
                    String confirm = sc.nextLine();
                    if (confirm.equalsIgnoreCase("yes") || confirm.equalsIgnoreCase("Yes")
                            || confirm.equalsIgnoreCase("y") || confirm.equalsIgnoreCase("Y"))
                        return list.transferMoney(curAccount, receiveAccountId, transferMoney);
                    else
                        return -3;
                }
            }
    ...
    public int removeAcc(BankAccount curAccount) {
        if (curAccount.getBalance() >= 1) {
            ...
            if (confirm.equalsIgnoreCase("yes") || confirm.equalsIgnoreCase("Yes") || confirm.equalsIgnoreCase("y")
                    || confirm.equalsIgnoreCase("Y"))
                return list.removeAcc(curAccount);
            else
                return -3;
        } else
            return -4;
